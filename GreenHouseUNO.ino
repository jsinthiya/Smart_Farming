...
void loop() {
  ...
  // Serial.print(F(" C Soil Sensor: "));
  // Serial.print(soilData.snesorValue);
  // Serial.print(F(" Soil Mosture Level: "));
  // Serial.print(soilData.level);
  // Serial.print(F(" Light Power: "));
  // Serial.print(lightData.sensorValue);
  // Serial.print(F(" Light Level: "));
  // Serial.print(lightData.level);
  bool window = false;
  bool humidifierStatus = humidifier(lightData, dht11data);
  if (humidifierStatus || lightData.level==1)
  {
    window = true;
...
DynamicJsonDocument doc(capacity);
  ...
  doc["greenWindow"] = window;

  doc["incubatorHumidity"] = 0;
  doc["incubatorTemperature"] = 0;
  doc["incubatorTurnner"] = false;
  doc["incubatorLight"] = false;
